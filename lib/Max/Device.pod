...
=head3 config_temperatures($comfort, $eco, $max, $min)

Configures the device temperature presets and on-device setpoint limits. To
keep any of the values unchanged, provide it as C<undef>. Will work on heaters
and wall thermostats, or emit a warning otherwise.
